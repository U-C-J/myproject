...
N_Cls = 10
inDir = dirname
DF = pd.read_csv(inDir + '/data/train_wkt_v4.csv')
GS = pd.read_csv(inDir + '/data/grid_sizes.csv', names=['ImageId', 'Xmax', 'Ymin'], skiprows=1)
SB = pd.read_csv(inDir+ '/data/sample_submission.csv')
ISZ = 160
smooth = 1e-12
...
def stick_all_train():
    ...
    for i in range(5):
        for j in range(5):
            ...
            img = M(id)
            img = stretch_n(img)#normalize to 0-1
...
def train_net():
    ...
    x_val = np.load(inDir+'/data/x_tmp_10.npy')
    print("read xval")
    ...
    model_checkpoint = ModelCheckpoint(inDir+'/data/weights/unet_tmp.hdf5', monitor='loss', save_best_only=True)
    for i in range(1):
        ...
        print ('val jk', score)
        model.save_weights(inDir+'/data/weights/unet_10_jk%.4f' % score)
...
def make_submit():
    ...
    print (df.head())
    df.to_csv(inDir+'/1.csv', index=False)


def check_predict(id='6120_2_3'):
    model = get_unet()
    model.load_weights('data/weights/unet_10_jk0.7878')
